...
import { Category, Yodda } from "@prisma/client";
...
interface YoddaFormProps {
  categories: Category[];
  initialData: Yodda | null;
};

export const YoddaForm = ({
  categories,
  initialData
}: YoddaFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      if (initialData) {
        await axios.patch(`/api/yodda/${initialData.id}`, values);
      } else {
        await axios.post("/api/yodda", values);
      }
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 pb-10">
          <div className="space-y-2 w-full col-span-2">
            <div>
              <h3 className="text-lg font-medium">Create your Yodda</h3>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>
                  Write a couple of examples of a human chatting with your AI character, write expected answers.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
